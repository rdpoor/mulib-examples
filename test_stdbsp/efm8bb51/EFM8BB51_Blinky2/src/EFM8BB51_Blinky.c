...
{
  // Disable the watchdog timer by writing 0xDE 0xAD in succession
  WDTCN = 0xde;
  WDTCN = 0xad;
}

static void test_stdbsp_init(void) {
  // UART0 P0.4 for TX, P0.5 for RX, baud set by T1 overflow
  //   Baud rates up to SYSCLK/2 (transmit) or SYSCLK/8 (receive)
  // BUTTON is P0.2 (low true)
  // LED is P1.4 (high true)
  SCON0 = SCON0_REN;  // enable UART0 receive
  _nop_();
}

static void test_stdbsp_step(void) {
  _nop_();
}
